...
import lamarao.jose.newsapp.ui.newsDetails.NewsDetailsDirections
...
class InitialScreen : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        viewModel.showPrompt.observe(viewLifecycleOwner, {
            if (it==true) {
        ...
        viewModel.navigateToMain.observe(viewLifecycleOwner, {
            if (it == true) {
                this.findNavController().navigate(
                    InitialScreenDirections.actionInitialScreenToMainFragment()
                )
    ...
    //verify if user has biometrics enabled and act accordingly. if user can authenticate set biometrics and show prompt else navigate to main activity fragment
    private fun checkBiometrics() {
        val biometricManager = BiometricManager.from(requireContext())
        when (biometricManager.canAuthenticate(BIOMETRIC_STRONG)) {
            BiometricManager.BIOMETRIC_SUCCESS ->{ setBiometrics(); viewModel.showPrompt() }
